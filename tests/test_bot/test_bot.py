...
class ConverterCommand:
    username = crescent.opt.string("username").convert(normalize_name)
    url1 = crescent.opt.string("url1").convert(fancy_validate_url)
    url2 = crescent.opt.string("url2").convert(fancy_validate_url)
...
class ClassCommand:
    arg = crescent.opt.string("description")
    another_arg = crescent.opt.string("another arg").name("another-arg")
    converted = crescent.opt.string("number as text").name("str-to-num").convert(lambda v: int(v))
