...
class ConverterCommand:
    username = crescent.options.string("username").convert(normalize_name)
    url1 = crescent.options.string("url1").convert(fancy_validate_url)
    url2 = crescent.options.string("url2").convert(fancy_validate_url)
...
class ClassCommand:
    arg = crescent.options.string("description")
    another_arg = crescent.options.string("another arg").name("another-arg")
    converted = (
        crescent.options.string("number as text").name("str-to-num").convert(lambda v: int(v))
    )
