...
def event(
    ...
    def hook(includable: Includable[CallbackT[Any]]) -> None:
        includable.app.event_manager.subscribe(event_type=unwrap(event_type), callback=event_callback)

    def on_remove(includable: Includable[CallbackT[Any]]) -> None:
        includable.app.event_manager.unsubscribe(event_type=unwrap(event_type), callback=event_callback)
