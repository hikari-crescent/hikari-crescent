...
class CommandHandler:
    ...
    async def __post_guild_commands(
        ...
        try:
            ...
            await self._client._app.rest.set_application_commands(
            ...
            if not isinstance(self._client._app, CacheAware):
                return

            if guild in self._client._app.cache.get_guilds_view():
                _log.warning(
    ...
    async def purge_commands(
        ...
        *guilds: SnowflakeishOr[PartialGuild],
        ...
        if not skip_global or purge_everything:
            await self._client._app.rest.set_application_commands(self._application_id, ())
        ...
        for guild in guilds_to_purge:
            await self._client._app.rest.set_application_commands(self._application_id, (), guild)
    ...
    async def register_commands(self) -> None:
        ...
        await gather(
            self._client._app.rest.set_application_commands(
                application=self._application_id,
                # The only method that is called has been implemented.
